...
reportTeX = do
  ...
  document $ do
    ...
    raw "Программа вычисляет значение $R$, определяемое как $f(Z) + f(X + 1) + f(Y) + 1$. Начнем с рассмотрения функции $f$:"
    mathDisplay . ("f(x) = " <>) $ cases $ do
      raw "3x + L, & x < 0" >> lnbk
      raw "K, & 0 < x \\leqslant K" >> lnbk
      raw "3x + L, & x > K"
    raw "(где $L$ = 2943, $K$ = 52)"
    ...
    parbreak
    raw "Полученное выражение является первым ограничением, накладываемым на область допустимых значений параметров $Z, X, Y$, используемых в основной программе." >> newline
    raw "$R$, подобно параметрам, является знаковым числом, представленным 16 разрядами. Соответственно, на формулу распространяется следующее ограничение:"
    flalignstar $ do
    ...
    mathDisplay $ cases $ do
      ...
      raw "-2^{15} - 1 \\leqslant f(Z) + f(X + 1) + f(Y) \\leqslant 2^{15} - 2"
    "Выделим такой диапазон значений отдельной переменной, в котором вычислительной ошибки не возникнет независимо от значений остальных параметров:"
    flalignstar $ do
