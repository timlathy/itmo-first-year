...
reportTeX = do
  ...
  document $ do
    ...
    enumerate $ do
      ...
      item Nothing <> raw "Изменить значения точек останова по адресам 002 и 211 на HLT"
    ...
    enumerate $ do
      item Nothing <> raw "Загрузить комплекс программ в память базовой ЭВМ."
      item Nothing <> raw "Изменить значения точек останова по адресам 10C и 10F на HLT"
      item Nothing <> raw "Запустить основную программу в автоматическом режиме с адреса 100"
      item Nothing <> raw "Установить \"Готовность ВУ-2\""
      item Nothing <> raw "Дождаться останова"
      item Nothing <> raw "Записать содержимое аккумулятора в момент останова программы, а также значение ячейки, содержащей X."
      item Nothing <> raw "Продолжить выполнение программы и дождаться останова"
      item Nothing <> raw "Сравнить полученное значение с ожидаемым, основываясь на записанных значениях."
    vspace (Mm 4) <> parbreak <> textbf "Проверка работы функции, выводимой на ВУ-3" <> lnbk
    enumerate $ do
      item Nothing <> raw "Загрузить комплекс программ в память базовой ЭВМ."
      item Nothing <> raw "Изменить значения точек останова по адресу 135 на HLT"
      item Nothing <> raw "Запустить основную программу в автоматическом режиме с адреса 100"
      item Nothing <> raw "Установить \"Готовность ВУ-3\""
      item Nothing <> raw "Дождаться останова"
      item Nothing <> raw "Записать содержимое аккумулятора в момент останова программы, которое отражает значение функции, вычисленное из X."
